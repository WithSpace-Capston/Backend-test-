...
public class MemberController {
    ...
    public ResponseEntity<BasicResponse> getMember(@PathVariable("memberId") Long memberId) {
        ...
        if (!member.isPresent()) {
            return new ResponseEntity<>(new BasicResponse<>(0, "회원 조회 실패", null), HttpStatus.NOT_FOUND);
        } else {
            GetMemberResponseDto getMemberResponseDto = new GetMemberResponseDto(member.get());
